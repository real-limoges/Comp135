...
def main(args):
    ...
    l1 = []
    for item in range(0,len(training)):
        l1.append(training[item]['point'][0])
    ...
    for x in range(2,18):
        y = 1
        while x >y:
            instances.append(int((float(y)/float(x))*length))
            y = y+1
    

    for k in range(1,17):
        k_clusters(training,k, output,means,stdevs,instances)
...
def k_clusters(training, k, output, means, stdevs,instances):
    str1 = "k = " + str(k) + '\n'
    output.write(str1)
    sse = []
    runs = []
